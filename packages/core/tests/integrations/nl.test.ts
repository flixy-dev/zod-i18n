...
test("string parser error messages", () => {
  ...
  expect(getErrorMessage(schema.startsWith("foo").safeParse(""))).toEqual(
    'Ongeldige invoer: moet beginnen met "foo"'
  );
  expect(getErrorMessage(schema.endsWith("bar").safeParse(""))).toEqual(
    'Ongeldige invoer: moet eindigen met "bar"'
  );
  expect(getErrorMessage(schema.min(5).safeParse("a"))).toEqual(
    "Tekst moet minstens 5 karakter(s) bevatten"
  );
  expect(getErrorMessage(schema.max(5).safeParse("abcdef"))).toEqual(
    "Tekst mag niet meer dan 5 karakter(s) bevatten"
  );
  expect(getErrorMessage(schema.length(5).safeParse("abcdef"))).toEqual(
    "Tekst moet precies 5 karakter(s) bevatten"
  );
  expect(
    getErrorMessage(schema.datetime().safeParse("2020-01-01T00:00:00+02:00"))
  ).toEqual("Ongeldige datumtijd");
});
...
test("date parser error messages", async () => {
  ...
  expect(getErrorMessage(schema.safeParse("2022-12-01"))).toEqual(
    "Ongeldig type: tekst. Type datum vereist"
  );
  expect(
...
test("array parser error messages", () => {
  ...
  expect(getErrorMessage(schema.safeParse(""))).toEqual(
    "Ongeldig type: tekst. Type array vereist"
  );
  expect(getErrorMessage(schema.min(5).safeParse([""]))).toEqual(
    "Array moet minstens 5 element(en) bevatten"
  );
  expect(getErrorMessage(schema.max(2).safeParse(["", "", ""]))).toEqual(
    "Array mag niet meer dan 2 element(en) bevatten"
  );
  expect(getErrorMessage(schema.nonempty().safeParse([]))).toEqual(
    "Array moet minstens 1 element(en) bevatten"
  );
  expect(getErrorMessage(schema.length(2).safeParse([]))).toEqual(
    "Array moet precies 2 element(en) bevatten"
  );
});
...
test("function parser error messages", () => {
  ...
  ).toEqual("Ongeldig type: nummer. Type tekst vereist");
});

test("other parser error messages", () => {
  expect(getErrorMessage(z.literal(12).safeParse(""))).toEqual(
    "Ongeldige letterlijke waarde, vereist 12"
  );
  expect(getErrorMessage(z.enum(["A", "B", "C"]).safeParse("D"))).toEqual(
    "Waarde 'D' is ongeldig. Antwoordmogelijkheden zijn: 'A' | 'B' | 'C'"
  );
  expect(
    getErrorMessage(
      z
  ...
  ).toEqual("Onbekende sleutel(s) in object: 'cat', 'rat'");
  expect(
    getErrorMessage(
      z
